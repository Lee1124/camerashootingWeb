<!--输入框  支持远程搜索选择(自定义返回的条数)-->
<template>
  <div id="inputSearch">
    <div class="inputBox">
      <input class="input" type="text" v-model="inputValue" :readonly="isReadonly" :placeholder="inputSearchPlaceholder"
             @focus.stop="inputFocus" @click.stop="fn" @change="changeValue">
...
<script>
  ...
  let myMethods = {
    ...
    changeValue(){
      this.$emit('changeValue',{val:this.inputValue,obj:this.isSelectedItemObj})/*this.inputValue*/
    },
    //获取焦点
    inputFocus() {
      this.isShowList = true;
      this.$emit('inputFocus',{val:this.inputValue});//通过该方法进行数据请求
      this.$emit('changeValue',{val:this.inputValue,obj:this.isSelectedItemObj})
    },
    //创建
    createNewContent() {
  ...
  export default {
    name: "inputSearch",
    props: ['inputSearchPlaceholder','listData','isReadonly'],
    data() {
      return {
        inputValue: '',
        isShowList: false,
